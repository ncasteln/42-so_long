...
/*   Updated: 2023/07/13 17:41:43 by ncasteln         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

static void	redraw_exit(t_state *game)
{
	int	y;
	int	x;
...
void	move_player(t_state *game, int y, int x)
{
	...
	next_p_x = game->p.x + x;
	next_p_y = game->p.y + y;
	if (game->map[next_p_y][next_p_x] == 'E')
	{
		game->last_exit = 1;
		update_map(game, next_p_y, next_p_x);
	}
	else if (game->map[next_p_y][next_p_x] == 'C')
	{
		if (game->last_exit == 1)
		{
			game->last_exit = 0;
